...
Return server information and a list of the available processes.

Example request:
```bash
curl http://localhost:8000/?service=WPS&version=1.0.0&request=GetCapabilities
```
...
Return the parameters (names, data types) and output details for the given process.

Example request:
```bash
# If the process name contains spaces, use single-quotes and replace each space with "%20"
curl 'http://localhost:8000/?service=WPS&version=1.0.0&request=DescribeProcess&Identifier=Mangrove%20Cover%20Drill'
```
...
Send parameters to a given process and wait for the result.

Curl POST request
```bash
curl -H "Content-Type: text/xml" -d @wpsrequest.xml -X POST localhost:8000?service=WPS&version=1.0.0&request=Execute
```

Example request (file `wpsrequest.xml`):
```xml
<?xml version="1.0" encoding="UTF-8"?>
